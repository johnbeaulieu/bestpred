        ...
        if len(lp) > 0:
            ...
            if cowid != lastcowid or trait != lasttrait:
                if int(lasttrait) == int(plottrait) or int(plottrait) == 5:
                    ...
                    plottd_ = np.ma.array(tdy)
                    plottdy = np.ma.masked_where(plottd_ == threshold, plottd_)
                    plotla_ = np.ma.array(lac)
                    plotlac = np.ma.masked_where(plotla_ == -2.0, plotla_)
                    plotst_ = np.ma.array(std)
                    plotstd = np.ma.masked_where(plotst_ == threshold, plotst_)
                    plotdb_ = np.ma.array(dbp)
                    plotdbp = np.ma.masked_where(plotdb_ == threshold, plotdb_)
                    plotde_ = np.ma.array(dev)
                    plotdev = np.ma.masked_where(plotde_ == threshold, plotde_)
                    maxdim = plotdim.max()
                    ...
                    plot_title = "BP of %s for Cow %s (lactation %s)" % (
                        number2name[_trait],
                        _cowid,
                        _lacnum,
                    )
                    ...
                    pylab.axis((1, maxdim, 0.0, max(plotlac.max(), plotstd.max(), plottdy.max())))
                    figfile = "curve_%s_%s_%s.png" % (
                        _cowid,
                        number2name[_trait],
                        _lacnum,
                    )
